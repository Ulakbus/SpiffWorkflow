...
class _BpmnProcessSpecState(object):
    ...
    def get_dump(self):
        def recursive_dump(route_node, indent, verbose=False):
            ...
            dump = '%s (%s:%s)' % (task_spec.name, task_spec.__class__.__name__,
                                   hex(id(task_spec))) + '\n'
            if verbose:
                if task_spec.inputs:
                    dump += indent + '-  IN: ' + ','.join(
                        ['%s (%s)' % (t.name, hex(id(t))) for t in
                         task_spec.inputs]) + '\n'
            ...
            for i, t in enumerate(route_node.outgoing):
                dump += indent + '   --> ' + recursive_dump(t, indent + (
                '   |   ' if i + 1 < len(route_node.outgoing) else '       '))
            return dump
    ...
    def go(self, workflow):
        ...
        logging.debug('Leaf tasks after load, before _update_state: %s',
                      leaf_tasks)
        for task in sorted(leaf_tasks, key=lambda t: 0 if getattr(t,
                                                                  '_bpmn_load_target_state',
                                                                  Task.READY) == Task.READY else 1):
    ...
    def _go(self, task, route_node, leaf_tasks):
        ...
        if not route_node.outgoing:
            ...
            if not task._is_finished():
                if issubclass(task.task_spec.__class__,
                              SubWorkflow) and task.task_spec.spec.start in [
                    o.task_spec for o in route_node.outgoing]:
                    self._go_in_to_subworkflow(task, [n.task_spec for n in
                                                      route_node.outgoing])
                else:
                    self._complete_task_silent(task, [n.task_spec for n in
                                                      route_node.outgoing])
            for n in route_node.outgoing:
                matching_child = [t for t in task.children if
                                  t.task_spec == n.task_spec]
        ...
        while q:
            ...
            if not route[-1] == starting_route[-1]:
                if task_name and route[-1].name == task_name:
                    return route
                if transition_id and hasattr(route[-1],
                                             'has_outgoing_sequence_flow') and \
                    ...
                    if taken_routes:
                        ...
                        for taken in taken_routes:
                            t = taken.to_list() if not isinstance(taken,
                                                                  list) else taken
            ...
            for child in route[-1].outputs:
                ...
                if len(new_route) > 10000:
                    raise ValueError(
                        'Maximum looping limit exceeded searching for path to %s' % (
...
class CompactWorkflowSerializer(Serializer):
    ...
    def deserialize_workflow(self, s_state, workflow_spec=None, read_only=False,
                             **kwargs):
        """
    ...
    def _get_workflow_state(self, workflow):
        ...
        compacted_states = []
        for state in sorted(states, key=lambda s: ",".join(
                [s[0], s[2], (':'.join(s[1]))])):
            if state[-1] == 'R':
                state.pop()
            if state[-1] == []:
    ...
    def _restore_workflow_state(self, workflow, state):
        ...
        for state in state_list[:-1]:
            ...
            state = (
            Task.WAITING if len(state) > 2 and state[2] == 'W' else Task.READY)

            route, route_to_parent_complete = s.get_path_to_transition(
                transition, state, workflow_parents)
            routes.append((route, route_to_parent_complete, transition, state,
                           workflow_parents))
            ...
            for i in range(len(routes)):
                ...
                for j in range(len(routes)):
                    ...
                    if route.contains(other_route) or (
                        ...
                        taken_routes = [r for r in
                                        [r[0] for r in taken_routes] + [r[1] for
                                                                        r in
                                                                        taken_routes]
                                        if r]
                        route, route_to_parent_complete = s.get_path_to_transition(
